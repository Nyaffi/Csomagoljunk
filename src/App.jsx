...
function ListakOldal({ beall, onVissza }) {
  ...
  const [checked, setChecked] = useState(betoltChecked);
  useEffect(() => {
    try { localStorage.setItem(CHECKED_KEY, JSON.stringify(checked)); } catch (e) { /* localStorage nem elérhető */ }
  }, [checked]);
...
// ─── LOCALSTORAGE PERSZISZTENCIA ───────────────────────────────────────────────
// A böngésző saját tárolójában marad a beállítás és a bejelölt tétel-lista,
// amíg felül nem írja egy új mentés (pl. új utazás beállítása).
const BEALL_KEY = "utazasi-sos-beallitasok";
const CHECKED_KEY = "utazasi-sos-jeloltek";
...
function betoltBeall() {
  try {
    const mentett = localStorage.getItem(BEALL_KEY);
    if (mentett) return { ...ALAP_BEALL, ...JSON.parse(mentett) };
  } catch (e) { /* localStorage nem elérhető (pl. inkognitó mód) — alapértékkel indulunk */ }
  return ALAP_BEALL;
}

function betoltChecked() {
  try {
    const mentett = localStorage.getItem(CHECKED_KEY);
...
export default function App() {
  ...
  const [beall, setBeall] = useState(betoltBeall);

  // A beállítások minden változásnál automatikusan elmentődnek, így böngészőbezárás
  // vagy telefonváltás után is ott találod, ahol legutóbb abbahagytad.
  useEffect(() => {
    try { localStorage.setItem(BEALL_KEY, JSON.stringify(beall)); } catch (e) { /* localStorage nem elérhető */ }
  }, [beall]);
